...
<style>
:root{--bg-from:#1a3a6b;--bg-to:#2d5fa6;--text:#fff;--muted:rgba(255,255,255,0.65);--gold:#ffd700;--gold-bg:rgba(255,215,0,0.15);--gold-br:rgba(255,215,0,0.40);--card-bg:rgba(0,0,0,0.35);--card-bdr:rgba(255,255,255,0.16);--thead-bg:rgba(0,0,0,0.45);--row-bdr:rgba(255,255,255,0.10);--row-hover:rgba(255,255,255,0.06);}
body.dark{--bg-from:#0f172a;--bg-to:#1e293b;--card-bg:rgba(0,0,0,0.55);--card-bdr:rgba(255,255,255,0.10);}
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
body{font-family:'DM Sans','Segoe UI',sans-serif;background:linear-gradient(135deg,var(--bg-from) 0%,var(--bg-to) 100%);color:var(--text);min-height:100vh;}
.navbar{display:flex;justify-content:space-between;align-items:center;padding:14px 28px;background:rgba(0,0,0,0.40);backdrop-filter:blur(14px);border-bottom:1px solid rgba(255,255,255,0.12);position:sticky;top:0;z-index:200;gap:16px;}
.logo{font-size:18px;font-weight:700;display:flex;align-items:center;gap:9px;white-space:nowrap;}
.logo-icon{width:32px;height:32px;background:var(--gold-bg);border:1.5px solid var(--gold);border-radius:8px;display:flex;align-items:center;justify-content:center;}
.role-chip{background:rgba(26,58,107,0.60);border:1px solid rgba(100,160,255,0.40);color:#93c5fd;border-radius:20px;padding:4px 12px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;}
.nav-links{display:flex;gap:2px;}
.nav-links a{color:rgba(255,255,255,0.75);padding:6px 12px;border-radius:8px;text-decoration:none;font-size:13px;font-weight:500;transition:0.2s;}
...
.pill{background:rgba(0,0,0,0.30);border:1px solid rgba(255,255,255,0.14);border-radius:20px;padding:5px 11px;font-size:12px;display:flex;align-items:center;gap:5px;}
.pill i{font-size:10px;opacity:0.7;}
.icon-btn{width:32px;height:32px;background:rgba(0,0,0,0.30);border:1px solid rgba(255,255,255,0.14);border-radius:8px;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:13px;color:var(--text);}
.logout-btn{background:var(--gold);color:#1a0a00;border:none;border-radius:8px;padding:6px 14px;font-size:13px;font-weight:700;cursor:pointer;text-decoration:none;}
...
/* Section heading */
.sec-head{font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--muted);margin:28px 0 12px;display:flex;align-items:center;gap:8px;}
.sec-head::before{content:'';width:18px;height:2px;background:var(--gold);border-radius:2px;}

/* Report cards grid */
.reports-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:14px;}
.report-card{background:var(--card-bg);border:1px solid var(--card-bdr);border-radius:14px;padding:20px;display:flex;flex-direction:column;gap:10px;transition:transform 0.2s,border-color 0.2s;}
.report-card:hover{transform:translateY(-2px);border-color:rgba(255,215,0,0.40);}
.report-card-icon{width:44px;height:44px;border-radius:11px;display:flex;align-items:center;justify-content:center;font-size:18px;}
.report-card h3{font-size:14px;font-weight:600;}
.report-card p{font-size:12px;color:var(--muted);}
.report-card .stat{font-size:24px;font-weight:700;color:var(--gold);}
.btn-dl{background:var(--gold);color:#1a0a00;border:none;border-radius:8px;padding:9px 16px;font-size:12px;font-weight:700;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;gap:7px;transition:0.2s;margin-top:4px;}
.btn-dl:hover{background:#ffe040;}

/* Custom date range */
.range-card{background:var(--card-bg);border:1px solid var(--card-bdr);border-radius:14px;padding:22px 24px;margin-top:4px;}
.range-card h3{font-size:14px;font-weight:600;margin-bottom:16px;color:var(--gold);}
.range-form{display:flex;flex-wrap:wrap;gap:12px;align-items:flex-end;}
.range-field{display:flex;flex-direction:column;gap:6px;}
.range-field label{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.8px;color:var(--muted);}
.range-field input,.range-field select{background:rgba(0,0,0,0.35);border:1px solid rgba(255,255,255,0.25);border-radius:8px;padding:9px 13px;color:#fff;font-family:inherit;font-size:13px;outline:none;}
.range-field input:focus,.range-field select:focus{border-color:var(--gold);}

.table-card{background:var(--card-bg);border:1px solid var(--card-bdr);border-radius:14px;overflow:hidden;margin-top:4px;}
.table-card-header{padding:16px 22px;border-bottom:1px solid var(--card-bdr);display:flex;align-items:center;justify-content:space-between;}
.table-card-header h3{font-size:13px;font-weight:600;display:flex;align-items:center;gap:8px;}
.table-card-header h3::before{content:'';width:3px;height:14px;background:var(--gold);border-radius:2px;}
table{width:100%;border-collapse:collapse;font-size:13px;}
thead tr{background:var(--thead-bg);}
th{padding:11px 16px;text-align:left;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:rgba(255,255,255,0.85);}
td{padding:12px 16px;border-bottom:1px solid var(--row-bdr);color:#fff;font-weight:500;}
tbody tr:last-child td{border-bottom:none;}
tbody tr:hover{background:var(--row-hover);}
.badge{display:inline-flex;padding:3px 9px;border-radius:20px;font-size:11px;font-weight:700;}
.badge.g{background:rgba(74,222,128,0.15);border:1px solid rgba(74,222,128,0.35);color:#4ade80;}
.badge.y{background:rgba(251,191,36,0.15);border:1px solid rgba(251,191,36,0.35);color:#fbbf24;}
.charts-row{display:grid;grid-template-columns:1fr 1fr 0.7fr;gap:14px;margin-bottom:14px;}
.charts-row.extra{grid-template-columns:repeat(2,minmax(0,1fr));}
.chart-card{background:var(--card-bg);border:1px solid var(--card-bdr);border-radius:14px;padding:22px 20px;}
.chart-card h3{font-size:13px;font-weight:600;margin-bottom:18px;display:flex;align-items:center;gap:8px;}
.chart-card h3::before{content:'';width:3px;height:14px;background:var(--gold);border-radius:2px;display:inline-block;}
:root{--bg-from:#dff3ff;--bg-to:#b9e3ff;--text:#071827;--muted:#315166;--gold:#0b4f8a;--gold-bg:#d8efff;--gold-br:#000;--card-bg:rgba(255,255,255,0.82);--card-bdr:#000;--thead-bg:#c7e8fb;--row-bdr:#000;--row-hover:#d5ecff;--input-bg:#fff;--input-bdr:#000;}
body.dark{--bg-from:#dff3ff;--bg-to:#b9e3ff;--card-bg:rgba(255,255,255,0.82);--card-bdr:#000;}
.navbar,.pill,.icon-btn,.logout-btn,.report-card,.btn-dl,.range-card,.table-card,.chart-card,.range-field input,.range-field select{border-color:#000!important;}
.navbar{background:rgba(223,243,255,0.92);}
.nav-links a,.pill,td,th{color:#071827;}
.nav-links a:hover,.nav-links a.active,.icon-btn:hover,.logout-btn:hover,.btn-dl:hover{background:#073b78!important;color:#fff!important;border-color:#000!important;text-decoration:none;}
.report-card:hover,.range-card:hover,.table-card:hover,.chart-card:hover{background:#073b78!important;border-color:#000!important;color:#fff;}
.report-card:hover *,.range-card:hover *,.table-card:hover *,.chart-card:hover *{color:#fff!important;}
@media(max-width:900px){.charts-row,.charts-row.extra{grid-template-columns:1fr;}}
...
.pending-badge,
.badge.pending { background: #fff1b8 !important; color: #3d2f00 !important; border-color: #000 !important; }
.deliv-badge,
.badge.delivered { background: #dceec2 !important; color: #20360e !important; border-color: #000 !important; }
.low-badge { background: #ffe0d7 !important; color: #5b170d !important; border-color: #000 !important; }
