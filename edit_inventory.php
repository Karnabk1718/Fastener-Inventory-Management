...
$id = (int)$_GET['id'];
$editResult = mysqli_query($conn, "SELECT s.id, s.fastener_id, s.quantity FROM stock s WHERE s.id = $id");
...
if(isset($_POST['update'])) {
    ...
    if($qty <= 0) {
        ...
        mysqli_query($conn, "UPDATE stock SET fastener_id='$fid', quantity='$qty' WHERE id='$id'");
        header("Location: inventory.php?success=updated");
        exit();
    }
    $editData['fastener_id'] = $_POST['fastener_id'];
    $editData['quantity']    = $_POST['quantity'];
}
?>
<!DOCTYPE html>
<html lang="en">
...
<title>Edit Stock – Bolt Base</title>
...
<div class="main">
    <div class="page-header">
        ...
        <div>
            <h1>Edit Stock &nbsp;<span class="id-badge">#<?= $editData['id'] ?></span></h1>
            <p>Update the stock details below</p>
        </div>
    </div>
    ...
    <div class="form-card">
        <div class="form-card-title"><i class="fa fa-pen"></i> Edit Stock Details</div>
        <form method="POST">
            <div class="form-grid">
                <div class="field">
                    ...
                    <select name="fastener_id" required>
                        ...
                        while($f = mysqli_fetch_assoc($fl)):
                            $sel = ($editData['fastener_id'] == $f['id']) ? 'selected' : '';
                        ?>
