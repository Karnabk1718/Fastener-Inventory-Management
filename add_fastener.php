...
if(isset($_POST['add'])) {
    ...
    if(isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        if(!in_array($ext, $allowed)) {
            ...
            // Use __DIR__ so the path is always correct regardless of where PHP runs from
            ...
            if(!move_uploaded_file($_FILES['image']['tmp_name'], $uploadDir . $imageName)) {
                $errorMsg = 'Failed to save image. Check that the uploads/ folder exists and is writable.';
                $imageName = '';
            }
        }
    }
    ...
    if(!$errorMsg) {
        if($price <= 0) {
        ...
        } elseif(!valid_part_number($partNumber)) {
            $errorMsg = 'Part number cannot contain hashtags.';
            ...
            mysqli_query($conn, "INSERT INTO fastener(name,part_number,type,size,unit_price,description,image) VALUES('$name_safe','$part_safe','$type_safe','$size_safe','$price','$desc_safe','$imageName')");
            header("Location: fastener.php?success=added");
            exit();
        }
    }
}
...
<title>Add Fastener – Bolt Base</title>
...
<div class="main">
    <div class="page-header">
        ...
        <div>
            <h1>Add New Fastener</h1>
            <p>Fill in the details below to add a fastener</p>
        </div>
    </div>
    ...
    <div class="form-card">
        <div class="form-card-title"><i class="fa fa-plus"></i> Fastener Details</div>
            ...
            <div class="form-grid">
                ...
                <div class="field">
                    ...
                    <input type="text" name="part_number" placeholder="e.g. HHB-10"
                        pattern="[A-Za-z0-9._ -]+"
                        title="Letters, numbers, dots, underscores, hyphens, and spaces only. Do not use hashtags."
                        value="<?= htmlspecialchars($_POST['part_number'] ?? '') ?>">
                </div>
                <div class="field">
                    <label>Type</label>
                ...
                <div class="field full">
                    ...
                    <textarea name="description" placeholder="Brief description of the fastener…"><?= htmlspecialchars($_POST['description'] ?? '') ?></textarea>
                </div>
                <div class="field full">
                    <label>Image</label>
                    ...
                    <div class="img-preview-wrap" id="previewWrap">
                        ...
                        <div class="img-preview-label"><i class="fa fa-check-circle"></i> Image ready to upload</div>
                    </div>
                </div>
                <div class="form-actions">
                    <button type="submit" name="add" class="btn-primary">
                        <i class="fa fa-plus"></i> Add Fastener
                    </button>
